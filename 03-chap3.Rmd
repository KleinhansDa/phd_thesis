...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_eom.html)
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/report_RDT.html)
...
On average, as it was shown before (section \@ref(res-ccounts)), there’s a significant increase in clusters deposited (\@ref(fig:rdtdepo)B). Also, neither _shroom3_++ nor _shroom3_$--$ CCs drastically change their position once they are deposited.
...
As shown in figure\@ref(fig:rdtreg)A-A’ neither speed not acceleration drastically differ throughout the complete course of migration. Speed drops from an initial ~75 $\mu$m/h to about ~30 $\mu$m/h for both _shroom3_++ and _shroom3_$--$ 17 h later (figure 3.3 A). Similarly, while there is a positive acceleration of almost ~2 $\mu$m/h peaking after ~2-3 hours, the remaining two peaks progressively get smaller (\@ref(fig:rdtreg)A’). 
While for the area no difference can be detected (\@ref(fig:rdtreg)B), interestingly the roundness is on average significantly reduced in _shroom3_$--$ pLLPs (\@ref(fig:rdtreg)C). This is also evident from the montage in \@ref(fig:rdtdepo)A. 
...
(ref:rdtcorr) Developmental interdependencies. Digits = pearson coefficient **A** Pearson correlation coefficient between rosettiness and speed of migration through time **B** Correlation between shrm3++ and shrm3$--$ at end of migration. 
...
knitr::kable(acdata, "latex", booktabs=T, align=c('l', 'l', 'c', 'c', 'c'), escape = F, 
...
Figure \@ref(fig:acshape)B’ illustrates the three developmental stages used here. For _shrm3_++ pLLPs two (\@ref(fig:acshape)A, 32 hpf) to four (\@ref(fig:acshape)A, 40 hpf) areas of constricting regions can be observed (XZ pane), in _shroom3_$--$ its three (\@ref(fig:acshape)A, 36 hpf) to seven (\@ref(fig:acshape)A, 40 hpf), confirming the results of rosette detection (\@ref(res-rdtdet)). While at 32 and 36 hpf the spread of those regions is wider in _shroom3_$--$, at 40 hpf they are smaller but also increased in number. When looking at the central rosette along the width (YZ pane), it seems like there is no considerable difference between _shroom3_++ and _shroom3_$--$ rosettes. In SU5402 treated embryos, neither in XZ nor in YZ constriction can be observed and the cells seem to be mostly retained in their mesenchymal character and columnar shape. In addition, while constriction seems isotropic in the shroom3++ rosette center, constriction in the _shroom3_$--$ rosette center cells appears anisotropic.
...
(ref:hcish) Expression of _deltaD_ and _atoh1a_ in the pLLP. Recorded in greyscale at brightfield at 20X Magnification. Images show background subtracted and de-noised EDFs of original Z-stack data.
...
The dataset generated consists of two groups, _shroom3_++ and _shroom3_$--$, where the segmented lateral line images are split into pLLP and CC (section \@ref(mat-ato-set)). Each time-lapse movie has a total duration of 18-20 hours, consists of about 54 timepoints and two channels (488 & 561 nm).\newline

```{r hcdatatab}
hcdata <- read.delim("data/tables/hcdata.txt")
knitr::kable(hcdata, "latex", booktabs=T, align=c('l', 'l', 'c', 'c', 'c'), escape = F,
             caption = 'Haircell specification dataset summary',
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_tl_tom.html)
...
While CC count is restored in heat-shocked, _shroom3_$--$, transgene carrying embryos, it also seems to reduce the number of CCs deposited in _shroom3_++ (\@ref(fig:rescshrm)A). For the rosettiness however, we still get a significant difference (\@ref(fig:rescshrm)B). Correlation between CC count and rosettiness for heat-shocked embryos without _hs:shr3v1_ transgene (\@ref(fig:rescshrm)C, _hs:shr3v1_ 00) is about the same as for embryos not heat-shocked (\@ref(fig:rescshrm)C, _hs:shr3v1_ ??), but inverse to heat-shocked embryos with transgene (\@ref(fig:rescshrm)C, _hs:shr3v1_ +?). Importantly, correlation is lost between heat-shocked _shroom3_$--$ embryos with transgene and not heat-shocked _shroom3_++ embryos (\@ref(fig:rescshrm)C, right panel). Table 3 summarizes the number of datapoints.\newline
...
```{r rescshrmtab, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/hsdata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Heatshock rescue dataset summary', escape = F, align=c('r', 'c', 'c', 'c', 'c')) %>%
  kable_styling(full_width = T) %>%
  kable_styling(font_size = 11) %>%
  collapse_rows(columns = 1, latex_hline = "none", valign = "top") %>%
...
```{r rescshrmsignif, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/hssignif_clus.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Heatshock statistics',
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_hsshrm3.html)
...
(ref:rescsu) Titration of Fgf inhibitor **A** LL lengths are reduced with increased concentrations of Fgf inhibitor **B** Groupwise comparison of the length normalized cluster count per concentration of Fgf inhibitor.
...
```{r rescsutab, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/sudata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Morphogen rescue dataset summary', escape = F, align=c('c', 'c', 'c', 'c', 'c', 'c'),
             col.names = c("genotype", "conc. ($\\mu$M)", "LLs", "genotype", "conc. ($\\mu$M)", "LLs")) %>%
  kable_styling(full_width = T) %>%
  kable_styling(font_size = 11) %>%
  collapse_rows(columns = 1, latex_hline = "none", valign = "top") %>%
  add_header_above(c("shroom3++" = 3, "shroom3--" = 3), bold=T) %>%
  collapse_rows(columns = 4, latex_hline = "none", valign = "top") %>%
  footnote(general = "concentrations in 0.1% DMSO")
```

A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_su.html)
...
(ref:rescrock) Titration of Rock inhibitor **A** LL lengths are reduced with increased concentrations of Rock inhibitor **B** Groupwise comparison of the length normalized cluster count per concentration of Fgf inhibitor.
...
```{r rescrocktab, out.width = "0.95\\textwidth", fig.cap="(ref:rescrock)", fig.scap="Titration of Rock inhibitor"}
data <- read.delim("data/tables/rkoutdata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Rockout rescue dataset summary', align=c('r', 'c', 'l'), escape=F,
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_rkout.html)
...
```{r resccal, out.width = "0.85\\textwidth", fig.cap="(ref:resccal)", fig.scap="Calyculin treatment"}
knitr::include_graphics("figures/results/06_rescues/caliculyn/rescue_cali.png")
```

```{r resccaltab, out.width = "0.95\\textwidth", fig.cap="(ref:resccal)", fig.scap="Calyculin treatment"}
data <- read.delim("data/tables/caldata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Calyculin rescue dataset summary', align=c('c', 'c', 'c'), escape=F,
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_clcln.html)
...
```{r resctemp, out.width = "0.75\\textwidth", fig.cap="(ref:resctemp)", fig.scap="Thermodynamic rescue"}
knitr::include_graphics("figures/results/06_rescues/temp/rescue_temp.png")
```

```{r resctemptab, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/tempdata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Thermodynamic rescue dataset summary', align=c('c', 'c', 'c'), escape=F,
...
```{r resctempsignif, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/tempsignif.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Thermodynamic rescue statistics',
             align = c('l','l','c','c','c')) %>%
  collapse_rows(columns = 1, latex_hline = "none", valign = "top") %>%
  kable_styling(full_width = T) %>%
  kable_styling(font_size = 8) #%>%
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_temp.html)
...
```{r rescatotab, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/atodata.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Atoh rescue dataset summary', align=c('c', 'c', 'c'), escape=F,
...
```{r rescatosignif, out.width = "0.95\\textwidth"}
data <- read.delim("data/tables/atosignif.txt")
knitr::kable(data, "latex", booktabs=T, caption = 'Atoh1a MO rescue statistics',
             align = c('l','l','c','c','c')) %>%
  collapse_rows(columns = 1, latex_hline = "none", valign = "top") %>%
  kable_styling(full_width = T) %>%
  kable_styling(font_size = 8) #%>%
...
A full data analysis report can be found on [github](https://github.com/KleinhansDa/reports/blob/master/b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3/clusters_ato-mo.html)