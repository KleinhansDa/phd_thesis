...
(ref:prollabs) Tracks of Mitosis. (Image data) Last timepoint of Z-projected time-lapse movie. (Labels) each dots is marks a mitotic events. Each color represents one mitosis. Each connection between two dots represents 6 min.
...
knitr::kable(data, "latex", booktabs=T) %>%
  kable_styling(full_width = T) %>%
  kable_styling(font_size = 11) %>%
  add_header_above(c("id labels" = 3, "metrics" = 2))
```
