...
//	Opening Dialog
	...
	Zorder = newArray("Bottom to Top", "Top to Bottom");
		...
		Dialog.addMessage("-------------------------- MACRO OPTIONS --------------------------");
			...
			Dialog.addCheckbox("Reconstruct Ferret lines", false); 				// ferrr
			Dialog.setInsets(0,20,0);
			Dialog.addMessage("3D Membrane Segmentation tolerance level:"); 
			Dialog.addSlider("Tol", 0, 50, 10); 								// tol
...
// 	###################### CHANNEL 1: Cell Measurements and Maps ###########################
		...
		// ######## ########################################################################### #########
			...
			run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
	   		//selectWindow(R);
	   		getDimensions(width, height, channels, slices, frames);
			newImage("FerretOMap", "8-bit black", width, height, nferr);
			run("Properties...", "channels=1 slices="+nferr+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
			FerretOMap = getTitle();
			...
			//run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments feret centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
			run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments feret centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
			run("3D Manager");
			wait(100);
			selectWindow(OMap);
			...
			for(k=0; k<nb; k++) {
				...
				waitForUser("object: "+ obj + "volume: " + V);
		...
		// ######## ########################################################################### #########
			...
			// ######## Crop Rosette ########
	   		...
	   		// ### MEASURE WHOLE ROSETTE MERGED ###
				...
				run("Dilate", "stack");
				run("Erode", "stack");
				run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
	   ...
	   //run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments mean_grey_value std_dev_grey_value feret minimum_grey_value maximum_grey_value centroid_(pix) centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments integrated_density mean_grey_value std_dev_grey_value mode_grey_value feret minimum_grey_value maximum_grey_value centroid_(pix) centroid_(unit) distance_to_surface centre_of_mass_(pix) centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy exclude_objects_on_edges_z sync distance_between_centers=10 distance_max_contact=1.80");
	   //run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments mean_grey_value std_dev_grey_value feret minimum_grey_value maximum_grey_value centroid_(pix) centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
	   //run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments mean_grey_value std_dev_grey_value mode_grey_value minimum_grey_value maximum_grey_value centroid_(pix) centroid_(unit) distance_to_surface centre_of_mass_(pix) centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
	   run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments feret centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
	   run("3D Manager");
	   wait(100);
	   Ext.Manager3D_AddImage();
