...
//	######################### REGISTRATION PARAMETERS ########################
	...
	// Create Minimum Thresholded Mask
		...
		run("Convert to Mask");
		saveAs("Tiff", metadir + name + "_ZP_AngleMask.tif");
//	############################### ANGLE ##########################
	//angle();
	...
	print("   Rotation angle: "+Angle);
		...
		saveAs("Tiff", metadir + name + "_ZP_R.tif");
		ZP = getTitle();
			//close(ZPMAX);
//	########################## CROPPING ############################## 
	roiManager("reset");
	...
	print(" Registering "+ORG+"...");
				...
				// save
				   ...
				   roiManager("reset");
				   run("Analyze Particles...", "include add");
				   rmcount = roiManager("count")-1; // since roi count starts from zero
				   roiManager('select', rmcount); // include selection of most right
		...
		selectWindow("Morphological Segmentation");
			...
			if (C1label==515) {
				if (decon) {
					...
					call("inra.ijpb.plugins.MorphologicalSegmentation.segment", "tolerance=5", "calculateDams=true", "connectivity=6"); //30ms exposure
				}
			}
			if (C1label==-139) {
				if (decon) {
