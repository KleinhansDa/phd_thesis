...
//	Opening Dialog
	...
	Zorder = newArray("Bottom to Top", "Top to Bottom");
		...
		Dialog.addMessage("~~~~~~~~~~~~~~~ MEASUREMENTS ~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			...
			Dialog.addCheckbox("Measure Cell Constriction", false); 				// 18. cellAC
	...
	if (cellAC) {
		Dialog.create(header);
		Dialog.addCheckbox("Measure ACI fixed delta", false); 						// 19. fixAC
			Dialog.addSlider("µm", 1, 10, 2);										// 20. cellum
		Dialog.addCheckbox("Measure ACI symmetric delta", false); 					// 21. symAC
		...
		//cellseg = Dialog.getChoice();												// 24. cellseg
	}
//	CHOOSE DIRECTORIES
	if (decon) {
...
//	##################################################################################################################
	...
	//	Create subdirectories
			...
			datcelldir = pardir + File.separator + "03 - Measurements" + File.separator + "XYZ_Ferets" + File.separator;
	...
	//	PRINT METADATA TO LOG WINDOW
		...
		print("Macro options:");
			...
			if (cellAC) {
				...
				if (symAC) {
					print("    Symmetric ACI: true, "+cellum*100+" %");} else {print("    Symmetric ACI: false");
					}
			} else {
			...
			// check if there are actually any points
				List.setMeasurements; 
 				...
 				if (mean > 0) {
					...
					saveAs("XY Coordinates", datdir + name + "_R" + pos + "_XY.txt");
					getSelectionCoordinates(xpoints, ypoints);
					FX = xpoints;
					FY = ypoints;
					...
					wait(100);
					saveAs("results", datdir + name + "_Dist_R" + pos + ".txt");
	   				results();
