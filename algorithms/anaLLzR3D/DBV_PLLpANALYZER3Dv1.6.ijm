...
//	Opening Dialog
	...
	labelsC2 = newArray("H2BRFP", "Arl13b", " ");
		...
		Dialog.addMessage("                           MACRO OPTIONS");
			...
			Dialog.addCheckbox("Register rosettes", false);
		Dialog.setInsets(0,20,0);
		Dialog.addMessage("Membrane Segmentation tolerance level:");
			Dialog.addSlider("", 0, 50, 10);
...
// 	###################### CHANNEL 1: Cell Measurements and Maps ###########################
	...
	run("16-bit");
	   ...
	   //save objects map
	   	if (multi) {
			saveAs("Tiff", C1dir + name + "-C1_RC_3DM_O-Map.tif");
			} else {
				saveAs("Tiff", posdir + name + "_RC_3DM_O-Map.tif");
			}
	   OMap = getTitle();
	close(C1CB);
		// ######## Crop around rosette center positions #########
	   if (ros){
	   	print("---- registering regions of apical constriction ----");
	   	//roiManager("Open", metadir + name + "_Rosettes.zip");
	   	//run("Measure");
		radius=80; 
		if (Rlx > 0) {
			results();
	   		for (j = 0; j < Rlx; j++) { //RosN = Rosette Number
	   			...
	   			n = nSlices();
	   			run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
	   			saveAs("Tiff", posdir + name + "_RC_3DM_O-Map" + "_R" + pos + ".tif");
	   			R = getTitle();
	   			//close(R);
	   			close(OMapD);
	   		// ######## Draw Feret lines ########
				...
				//	Create substack
					getDimensions(width, height, channels, slices, frames);
					var done = false; 
					for(l=1; l<slices &&!done; l++) { // delete empty slices
						setSlice(l);
						max = getMinAndMax(min, max);
							if(max > 0) {
							done = true;
							s1 = getSliceNumber();
							}
					...
					n = nSlices();
					run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
					saveAs("Tiff", posdir + name + "_RC_3DM_O-Map" + "_R" + pos + "_FerretLines" + ".tif");
					FerretS = getTitle();
						...
						for (m = 0; m < Ferrlx ; m++) {
							if (m==0) {Ferrlx = Ferrlx;} else {Ferrlx = Ferrlx-1;}
								for (n = 0; n < Ferrlx; n++) {
									...
									//point = lx-n;
									//print("lx / pos / point = "+lx+" / "+pos+" / "+point);
									//print("sqrt("+"("+xpoints[lx]+" - "+xpoints[lx-pos]+")^2"+"+"+"("+ypoints[lx]+" - "+ypoints[lx-pos]+")^2)");
									if (ED > 0){
										Dist = Array.concat(Dist, ED);
										}
							//Dist[m] = sqrt((FX[Ferrlx]-FX[Ferrlx-m])*(FX[Ferrlx]-FX[Ferrlx-m])+(FY[Ferrlx]-FY[Ferrlx-m])*(FY[Ferrlx]-FY[Ferrlx-m]));
								}
						}
					} // if(mean) <0
					if (max == 0) {
						...
						setResult("Min", j, "NaN");
						setResult("Max", j, "NaN");
						setResult("Mean", j, "NaN");
						setResult("StdDev", j, "NaN");
						updateResults();
						} else {// if(mean<0)
				...
				updateResults();
	   			saveAs("results", datdir + name + "_Rosettes_Results" + ".txt");
	   			results();
	   		} // if (Rlx > 0)
	   } // if(ros)
	   //waitForUser("3");
    ...
    // CREATE COMPOSITE
       selectWindow(C1BC);
	   	...
	   	selectWindow("Composite");
	   		if (multi) {
	   		saveAs("Tiff", C1dir + name + "-C1_RC_3DM_OMap_Composite.tif");
				} else {
				saveAs("Tiff", posdir + name + "_RC_3DM_OMap_Composite.tif");
			}
	   close();
