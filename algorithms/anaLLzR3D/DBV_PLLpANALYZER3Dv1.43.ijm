		...
		if (Rlx > 0) {
			results();
	   		for (j = 0; j < Rlx; j++) { //RosN = Rosette Number
	   			...
	   			n = nSlices();
	   			run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
	   			saveAs("Tiff", posdir + name + "_RC_OMap" + "_R" + pos + ".tif");
	   			R = getTitle();
	   			//close(R);
	   			close(OMapD);
	   		// ######## Draw Feret lines ########
					...
					Ext.Manager3D_Close();
					selectWindow(Ferret);
					saveAs("Tiff", posdir + name + "_RC_OMap" + "_R" + pos + "_FerretLines" + ".tif");
					FerretS = getTitle();
				...
				// ######## get coordinates or feret lines @ .25% of the stack size ########
					...
					setSlice(um);
						...
						run("Duplicate...", " ");
						if (j == 0) {
							run("Label...", "format=Text starting=0 interval=[] x=5 y=20 font=18 text=[Z "+um+"] range=1-1");
						}
