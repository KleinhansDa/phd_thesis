...
// 	###################### MORPHOLOGICAL SEGMENTATIONs ###########################
		...
		//	MEASURE APICAL
			...
			//	Get Angle
				apAngle[k] = getResult("FeretAngle", 0);
				//print("meanline: "+meanline+", CY: "+cy);
				if (apAngle[k] > 90) {
				//	apAngle[k] = apAngle[k]+180;
				//}
				apAngle[k] = 90-(apAngle[k]-90);
				}
		...
		//	CROP & SAVE
				...
				Ext.Manager3D_Close();
				if (ACM=="fit") {
				Array.show("Results (row numbers)", objlabelArray, apFMax, apFMin, apAngle, apd, baFMax, baFMin, baAngle);
				}
				if (ACM=="bounding") {
				Array.show("Results (row numbers)", objlabelArray, apbw, apbh, apAngle, babw, babh, baAngle);
				}
