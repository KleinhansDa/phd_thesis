...
//	Opening Dialog
	...
	ACMeasure = newArray("feret's", "bounding", "area");
	Zorder = newArray("Bottom to Top", "Top to Bottom");
		...
		Dialog.addMessage("~~~~~~~~~~~~~~~ MEASUREMENTS ~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			...
			Dialog.addCheckbox("Measure ACI fixed delta", false); 				// cellAC
			Dialog.addSlider("µm", 1, 10, 2);									// cellum
			Dialog.addCheckbox("Measure ACI symmetric delta", false); 			// cellACd
			Dialog.addSlider("%", 10, 40, 20); 									// cell%
			Dialog.setInsets(0,20,0);
			Dialog.addChoice("AC measurement", ACMeasure);						// ACM
		...
		Dialog.show();
			...
			ACId = Dialog.getCheckbox();										// cellACd
			if (ACId) {															// cell%
				cellum = Dialog.getNumber();									
				cellum = cellum/100;
			}
			ACM = Dialog.getChoice();											// ACM
	selectWindow(header);
	wait(100);
	run("Close");
//	CHOOSE DIRECTORIES
...
//	##################################################################################################################
	...
	//	PRINT METADATA TO LOG WINDOW
		...
		print("Macro options:");
			...
			if (cellAC) {print("   Cell Constriction: true, "+cellum+" %");} else {print("   Cell Constriction: false");}
...
// 	###################### MORPHOLOGICAL SEGMENTATIONs ###########################
		...
		//	set multiselect
			...
			for(k = 0; k < nb; k++) {
				...
				if (ACId) { // if dynamic ACI
					Ext.Manager3D_Measure3D(k,"Zmax (pix)",zmax);
					Ext.Manager3D_Measure3D(k,"Zmin (pix)",zmin);
					cheight = zmax-zmin;
					da = cheight*cellum;
					round(da);
				}
		...
		//	MEASURE APICAL
			...
			//	Max Feret / bounding width / area
				resultsArray = newArray(nResults());
				for(p = 0; p < nResults(); p++) { 
					resultsArray[p] = getResult("Width", p);	
  				}
  				total = 0; 
				for(p = 0; p < nResults(); p++) { 
			...
			//	Min Feret / bounding height / area
				resultsArray = newArray(nResults());
				for(p = 0; p < nResults(); p++) { 
       				resultsArray[p] = getResult("Height", p); 
  				}
  				total = 0; 
				for(p = 0; p < nResults(); p++) { 
 					total = total + resultsArray[p]; 
		...
		//	CROP & SAVE
				...
				Ext.Manager3D_Close();
				if (ACM=="feret's") {
				Array.show("Results (row numbers)", objlabelArray, apFMax, apFMin, baFMax, baFMin);
				}
				if (ACM=="bounding") {
				Array.show("Results (row numbers)", objlabelArray, apbw, apbh, babw, babh);
				}
