		...
		// Save
		   ...
		   if (multi) {
		   	saveAs("Tiff", C1dir + name + "-C1_RC.tif");
		   		...
		   		saveAs("Tiff", pdir + name + "-C1_RC.tif");}
	...
	wait(2000);
		selectWindow("Morphological Segmentation");
			...
			if (C1label==515) {
				...
				call("inra.ijpb.plugins.MorphologicalSegmentation.segment", "tolerance=400", "calculateDams=true", "connectivity=6"); //20ms exposure
			} 
			if (C1label==-139) {
	...
	wait(90000);
			...
			if (multi) {
		   		saveAs("Tiff", C1dir + name + "-C1_RC_WS.tif");
		   			...
		   			saveAs("Tiff", pdir + name + "-C1_RC_WS.tif");}
			C1WS = getTitle();
		selectWindow("Morphological Segmentation");
		close();
		selectWindow(C1F);
...
// 	###################### Channel 1: Cell Measurements and Maps ###########################
	...
	// save surface and objects map
	   selectWindow("Surface map of "+C1WS);
	   	if (multi) {
			saveAs("Tiff", C1dir + name + "-C1_RC_WS_3DOC_S-Map.tif");
			} else {
				saveAs("Tiff", pdir + name + "_RC_WS_3DOC_S-Map.tif");
			}
	   ...
	   selectWindow("Objects map of "+C1WS);
			...
			saveAs("Tiff", C1dir + name + "-C1_RC_WS_3DOC_O-Map.tif");
	   		...
	   		//run("Enhance Contrast...", "saturated=0 equalize process_all");
	   			// measure with 3DManager
	   			   ...
	   			   Ext.Manager3D_SaveResult("M", resultsdir + name + "_3DMngr_CellMeasure.csv");
	   	...
	   	selectWindow("Centroids map of "+C1WS);
			...
			saveAs("Tiff", C1dir + name + "-C1_RC_WS_3DOC_C-Map.tif");
	   		...
	   		close();
	// save Results
	   //saveAs("results", C1Imgdir + name + "_3DOC_Results" + ".txt");
    ...
    // Create composite
	   ...
	   selectWindow("Composite");
	   		...
	   		saveAs("Tiff", C1dir + name + "-C1_RC_WS_OMap_Composite.tif");
				...
				saveAs("Tiff", pdir + name + "-C1_RC_WS_OMap_Composite.tif");
			}
	   close();
