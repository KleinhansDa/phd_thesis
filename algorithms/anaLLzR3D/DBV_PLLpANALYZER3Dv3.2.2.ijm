...
//	Opening Dialog
	...
	Zorder = newArray("Bottom to Top", "Top to Bottom");
		...
		Dialog.addMessage("~~~~~~~~~~~~~~~ PROCESSING ~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			...
			Dialog.addCheckbox("Save metadata", false); 							// 07. meta
			Dialog.setInsets(0,20,0);
			Dialog.addChoice("Z order", Zorder); 									// 08. rev
			Dialog.addCheckbox("Register primordium", true); 						// 09. reg
			Dialog.addCheckbox("Reconstruct Ferret lines (EXPERIMENTAL)", false); 	// 10. ferrr
			Dialog.addCheckbox("Manual segmentation", false); 						// 11. manual
		Dialog.setInsets(0,10,0);
		Dialog.addMessage("~~~~~~~~~~~~~~~~ THRESHOLDS ~~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			Dialog.setInsets(0,20,0);
			Dialog.addMessage("3D Membrane Segmentation tolerance level:"); 
			Dialog.addSlider("Tol", 0, 50, 5); 										// 12. tol
			Dialog.addMessage("Min. Volume threshold");							
			Dialog.addSlider("Min.", 0, 50, 20);									// 13. vmin
			Dialog.addMessage("Max. Volume threshold");							
			Dialog.addSlider("Max.", 250, 1000, 750);								// 14. vmax
		Dialog.setInsets(0,10,0);
		Dialog.addMessage("~~~~~~~~~~~~~~~ MEASUREMENTS ~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			Dialog.addCheckbox("Measure Rosette Constriction", false); 				// 15. rosAC
			Dialog.addCheckbox("Measure Cell Constriction", false); 				// 16. cellAC
		//info();
		Dialog.show();																// ## get params ##
			...
			meta = Dialog.getCheckbox(); 											// 07. meta
			rev = Dialog.getChoice();												// 08. rev
			reg = Dialog.getCheckbox();												// 09. reg
			ferrr = Dialog.getCheckbox();											// 10. ferrr
			manual = Dialog.getCheckbox();											// 11. manual
			tol = Dialog.getNumber();												// 12. tol
			vmin = Dialog.getNumber();												// 13. vmin
			vmax = Dialog.getNumber();												// 14. vmax
			rosAC = Dialog.getCheckbox();											// 15. rosAC
			cellAC = Dialog.getCheckbox();											// 16. cellAC
	selectWindow(header);
	wait(100);
	run("Close");
	...
	if (cellAC) {
		...
		Dialog.addCheckbox("Relative ACI delta", false); 							// 21. symAC
...
//	##################################################################################################################
	...
	//	PRINT METADATA TO LOG WINDOW
		...
		print("Macro options:");
			...
			if (cellAC) {
				print("   Cell Constriction: true");
				if (fixAC) {
					print("    Fix ACI: true, "+cellum+" µm");} else {print("    Fix ACI: false");
					}
				if (symAC) {
					print("    Relative ACI: true, "+cellum*100+" %");} else {print("    Symmetric ACI: false");
					}
			} else {
				print("   Cell Constriction: false");
			}
