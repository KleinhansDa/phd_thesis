...
//	########################## CROPPING ############################## 
	...
	print(" Registering "+ORG+"...");
				...
				// Crop
				   ...
				   saveAs("Tiff", pdir + name + "_ZPMAX_RC.tif");
				   ZPMAX = getTitle();
		...
		selectWindow("Morphological Segmentation");
			...
			if (C1label==515) {
				if (decon) {
					...
					call("inra.ijpb.plugins.MorphologicalSegmentation.segment", "tolerance=2", "calculateDams=true", "connectivity=6"); //30ms exposure
				}
			}
			if (C1label==-139) {
				if (decon) {
