...
// ############### Rosette Detection ##############
				...
				// Find Maxima
				   	...
				   	print("Number of Rosettes: "+lengthOf(xpoints));
				   	roiManager("Add");
				...
				// Save capture
					...
					run("Point Tool...", "type=Circle color=Red size=[Extra Large] label counter=0");
				   	//run("Capture Image");
				   	saveAs("Tiff", posdir + name + "_ZPMAX_RC_RosetteLabel.tif");
				   	ZPSTDD = getTitle();
				   	roiManager("reset");
				   	close(ZPSTDClose);
				   	close(ZPSTDD);
...
// ######## Crop around rosette center positions #########
	   if (ros){
	   	...
	   	//run("Measure");
		radius=100; 
		if (lengthOf(xpoints)>0) {
	   		for (j = 0; j <= lengthOf(xpoints); j++) { //RosN = Rosette Number
	   			// create dir
	   			pos = j+1;
	   			selectWindow(OMap);
	   			run("Duplicate...", "duplicate");
	   		...
	   		// ######## Draw Feret lines ########
				...
				//	Create substack
					...
					n = nSlices();
					run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
					saveAs("Tiff", posdir + name + "_RC_3DM_O-Map" + "_R" + pos + "_FL" + ".tif");
					FerretS = getTitle();
				...
				// ######## Feret calculations ########
					...
					getSelectionCoordinates(xpoints, ypoints);
					lx = lengthOf(xpoints)-1;
					Dist = newArray(lx);
					for (m = 0; m < lx; m++) {
						Dist[m] = sqrt((xpoints[lx]-xpoints[lx-m])*(xpoints[lx]-xpoints[lx-m])+(ypoints[lx]-ypoints[lx-m])*(ypoints[lx]-ypoints[lx-m]));
						}
	   			...
	   			saveAs("results", datdir + name + "_R" + pos + "_FL_Results" + ".txt");
	   			results();
	   		}
	   }
	   //waitForUser("3");
