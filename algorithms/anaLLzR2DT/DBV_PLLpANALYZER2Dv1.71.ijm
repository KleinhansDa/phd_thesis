...
// OPENING DIALOG
	...
	Dialog.setLocation(DiaW,DiaH);
		...
		Dialog.addChoice("Direction of Migration", Migration);
...
// --- Variables and Identifiers Dialog Box ---
	...
	for (j = 0; j < orgdirlist.length; j++){
		Dialog.addString("Position: "+ orgdirlist[j], "??", 6);
	}
...
if (pre) {
	...
	//waitForUser("294 before segmentation");
	//  #####################################################################################
	//	################################### STACK SEGMENTATION ####################################
	//  #####################################################################################
...
print("  Segmenting "+embryoID+"_RC...");
	...
	//	########################## BLURRING ##############################
		//print("Blurring...");
		selectWindow(MC);
		run("Gaussian Blur...", "sigma=4 scaled stack"); // MC
...
wait(200);
		...
		roiManager("Sort");
		//waitForUser("WAIT ROIS renamed and sorted?");
		//selectWindow(RC);
		...
		//waitForUser("HAL");
			...
			selectWindow(RC);
			//waitForUser("WAIT 467 Check ROI Manager");
			roiManager("show none"); // supress roimanager popping up
				roiManager("Select", m);
