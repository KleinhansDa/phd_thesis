...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	run("Clear Results");
	// disable IJ logging
	//enableIJLogging(false)
	//./ImageJ-windows10 --ij2 --headless
...
// 	### Variables and identifiers dialog box ###
	...
	// --- Variables and Identifiers Dialog Box ---
	showStatus("STEP 2/3");
	Dialog.create("pLLP ANALYZER v0.6 - STEP 2/3");
	Dialog.setLocation(300,200);
	...
	Dialog.create("pLLP ANALYZER v0.6 - STEP 3/3");
	Dialog.setLocation(300,200);
	...
	for (j = 0; j < binlength; j++){
		...
		//print(c);
			...
			Dialog.addString("Position "+d2s(0,0)+d2s(c,0), "??", 4);
...
function plot() {
	if (b==1){
	Plot.create([embryo]+" L.E. / Time", "Time [h]", "L.E. [µm]", Time, LE);
	} else {
		Plot.add("line", Time, LE)
	}
}
