...
print("Saving: "+embryoID+"_RC");
	...
	// rotate and crop ZPAVG for image calc
	selectWindow(ZPAVG);
...
print("Segmenting "+embryoID+"_RC...");
	...
	//	Morphological closing
		//run("Morphological Filters (3D)", "operation=Closing element=Ball x-radius=5 y-radius=5 z-radius=0");
		print("Morphological closing 3D...");
		selectWindow(BC);
		run("Morphological Filters (3D)", "operation=Closing element=Ball x-radius=20 y-radius=20 z-radius=0");
		MC = getTitle();
		close(BC);
	//	########################## BLURRING ##############################
		//print("Blurring...");
