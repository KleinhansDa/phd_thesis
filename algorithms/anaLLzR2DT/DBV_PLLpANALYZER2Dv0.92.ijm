...
// OPENING DIALOG
	...
	Dialog.create(header+"- STEP 1/3");
	Dialog.setLocation(DiaW,DiaH);
	  Dialog.addMessage("Before we can start, we need to define some parameters first...\nNext you will be guided through 3 consecutive steps.");
	  Dialog.addMessage("STEP   I: Does your data need pre-processing? \n(rotation, cropping, segmentation)");
	  	  Dialog.addCheckbox("Yes, it does!", true);
	  Dialog.addMessage("STEP  II: Input and output directory definition \nSTEP III: Genotype definition \n ");
...
// 	Input and Output directories
	if (pre) {
		showStatus("STEP 1/2: 8-/16-bit input");
...
if (pre) {
	...
	//	Morphological closing
		...
		run("Morphological Filters (3D)", "operation=Closing element=Cube x-radius=5 y-radius=5 z-radius=0");
		MC = getTitle();
		close(BC);
	//	Blurring
		print("Blurring...");
		run("Gaussian Blur...", "sigma=3.5 scaled stack"); // still BC
	...
	setSlice(n);
	run("Convert to Mask", "method=Otsu background=Default");
