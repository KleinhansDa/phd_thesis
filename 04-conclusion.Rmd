...
```{r klein_2019_conc, out.width = '95%', fig.keep = 'all', fig.show = 'hold'}
knitr::include_graphics("files/pubs/klein_2019/006_02.jpg")
```
...
```{r klein_2019_disc, out.width = '95%', fig.keep = 'all', fig.show = 'hold'}
...
Due to their high regenerative ability, zebrafish are a popular model in regenerative research [@Chitnis2012a; @Mokalled2016; @Harding2014a]. It is well known that zebrafish NMs have a very high regenerative capacity [@Cruz2015; @Pinto-Teixeira2015; @Wada2013].
...
In the pLLP, Notch signaling is important for selection and specification of hair-cells (\@ref(intro-notch)). Interestingly, our findings are supported by another study where it was shown that upon over-activated Notch signaling, CC deposition happened independent of Shroom3 [@Kozlovskaja-Gumbriene2017]. For our observation this could mean that **(1)** either expression of _atoh1a_ is up-regulated due to a compensation mechanism to ensure CC deposition or **(2)** that the cellular rearrangements lead to biased Notch signaling. A phenomenon that in fact has been well studied [@Shaya2017a]. For the latter, the proposed model would be that in each micro-rosette the HC is in contact with less cells, but the amount of Notch ligand stays the same. Therefore, lateral inhibition and feedback is stronger, leading to an increase in expression of _atoh1a_ and ultimately to premature CC deposition.
...
(ref:dismap) pLLP Maps. The QR code links to the LLMapR web-app (dskleinhans.shinyapps.io/LLmapR).

```{r dismap, out.width='95%', fig.cap="(ref:dismap)", fig.scap="pLLP Maps"}
...
(ref:dispca) Principal Component Analysis **A and B** show dimension 1 (explaining 42.6$\%$ of the data) and dimension 2(explaining 23.3$\%$ of the data) in a scatter plot. The bigger dots represent the respective centers. Ellipsoids show the spread of the data. In addition **B** shows the individual principal components. The arrows direction is the PCs direction, the length represents its importance.

```{r dispca, out.width='85%', fig.cap="(ref:dispca)", fig.scap="Principal Component Analysis"}
