...
# ###  ENTER 2nd LOOP TO INCREMENT OVER EACH SLICE OF THE TIME-SERIES ###
  ...
  #   Prim registration
    ...
    # 	Euclidian Distance of X + normalized to offspring 'zero'
    		...
    		setResult("LE", r, LE);
    		updateResults();
        # Velocitiy LE (LE1 = LE @ timepoint 1; 
    		# LEN = normalized value of LE, 
    		# LENV = Velocity of the normalized value of LE)
    		if (i == 1) {
    		# LE1 will be the same for all further timepoints
    			LE1 = LE; 
    			LEN = 0;
    		# 'Leading Edge Normalized Euclidian Distance'
    			LENED = NaN;
    		# For the first timepoint there can be no speed, 
    		# since there was no coordinate of X and Y before
    			LEV = NaN; 
    		} else {
    		# The value of 'LE Normalized' to zero
    			LEN = LE - LE1;
    		# LED = The value of LE one row before
    			LED = getResult("LE_N", r2);
    		# LENED = LEN - (LEN-LED);
    			LENED = sqrt((LEN-LED)*(LEN-LED)+(Y2-Y1)*(Y2-Y1)); 
    			LEV = LENED / time;
    		}
    	# setResult Leading Edge Normalized (LE_N)
    		setResult("LE_N", r, LEN);
    	# setResult Leading Edge Normalized Euclidian Distance (LE_N_ED)
    		setResult("LE_N_ED", r, LENED);
    	# setResult Leading Edge Velocity (LE_V)
    		setResult("LE_V", r, LEV); 
    		updateResults();
			}
...
```{r ACICells, out.width = '75%', fig.cap = "(ref:ACICells)", fig.scap = "ACI Cell Models"}
