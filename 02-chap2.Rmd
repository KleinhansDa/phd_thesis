...
The _Cluster Analysis_ algorithm was designed for semi-automatic cell cluster detection in the _cldnb:lyn-gfp_ transgenic line and optional nuclei counting in a second DAPI labeled channel within the Regions of Interest (ROIs) derived from the cell cluster detection.
...
For image analysis a custom IJ macro script was developed that segments individual cell clusters based on morphological filtering[@Legland2016], Gaussian blurring and subsequent thresholding. The user is then presented the segmentation ROIs to manually edit them, if necessary. Using those ROIs as masks, the nuclei within them are counted with a 2-D maxima finder. To detect the right amount of nuclei however, it is necessary to evaluate the distance over which the blurring should be applied.
...
(ref:GrTratioCC) Relative difference of segment counts (N=3, Error bars = Std./2, NB = nuclei blurring)
...
ggplot(NB_all_diff_mean, aes(Group, mean_diff)) +
  ...
  labs(title = "", x = "Blurring Parameters", y = "Rel. Count Diff. [%]") +
...
The anallzr3D algorithm was designed for fully automated, single cell volume segmentation in the _cldnb:lyn-gfp_ transgenic line. In addition to 3D cellular metrics, it offers Apical Constriction measurement of each cell.
...
```{r maxraw, out.width='90%', fig.cap = "(ref:maxraw)"}
...
```{r anallzrvols, out.width='85%', fig.cap = "(ref:stackcomp)"}
...
(ref:volsecdffull) Cell volumes full distribution (red line = .4 Quantile)
...
ggplot(all_vol, aes(Vol..unit., colour = Group)) +
  ...
  labs(title = "test", x = "Volumes", y = "Relative amount", color = "level") +
...
(ref:volsecdfzoom) Cell volumes distribution within .4 Quantile
...
Zebraﬁsh husbandry was maintained at the University of Frankfurt am Main. All legal procedures were followed while handling and maintaining zebraﬁsh husbandry. All zebraﬁsh lines used in and generated for this study are listed in the Appendix A.
...
1. **To obtain a sufficient amount of data that is closely staged and still comprises > 4 independent groups.** Using this protocol, it is possible to define a custom well plate (figure \@ref(fig:stampprod) C) in your preferred imaging software, which greatly facilitates the imaging procedure and therefore helps to stay focused. However, once the well plate is aligned and the positions are defined it still needs some minor correction in Z.
...
knitr::include_graphics("files/pubs/klein_2019/006_prep.jpg")
...
The method used for rosette detection is based on a convolutional neuronal network (CNN) and was modified from the "rosette detector" algorithm previously used in the lab and described in [@Ernst2012a; @Liu]. Since the former method was technically outdated and since we had new data in which we needed to detect rosettes, we updated the former method to a state of the art CNN using Caffe[@Jia2014] as a backend. The training was done by our collaborators at the institute for Informatics, Albert-Ludwigs-University Freiburg.

The CNN was trained on 17 DMSO- and SU5402- treated embryos each. SU5402 is an inhibitor of FGF receptors and embryos treated with these inhibitors show a strongly reduced number of rosettes ([@Lecaudey2008a]). In order to give the network something to learn, the data had to be labeled manually, which was done in ImageJ by placing multipoint ROIs at the center of the rosettes. The data was then further permutated to artificially increase the amount of training data and make the detection more robust against different kinds of input. Further parameters about the training data is listed in Table \@ref(tab:CNNtraining). One example for each is shown in figure \@ref(fig:CNNtrain).
...
```{r mitodatapoints, out.width='50%', fig.cap="(ref:mitodatapoints)", fig.scap = "Tracking of mitotic events"}
...
```{r ACLee, out.width = '75%', fig.cap = "(ref:ACLee)", fig.scap = "A.I. indeces in the literature"}
...
```{r, out.width = '85%', fig.cap = "(ref:ACIMinor)"}
...
```{r HardingACI, out.width='60%', dpi=200, fig.cap="(ref:HardingACI)", fig.scap = "AI indices by Harding et al."}
